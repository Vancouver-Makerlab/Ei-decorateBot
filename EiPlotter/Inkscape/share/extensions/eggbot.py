...
# Modified for EggPlotter bei Tom Frey
#
...
N_PAGE_HEIGHT = 900       # Default page height (each unit equiv. to one step)
N_PAGE_WIDTH = 4076       # Default page width (each unit equiv. to one step)
	...
	def plotLineAndTime( self ):
		...
		if self.bPenIsUp:
			...
			if ( self.options.wraparound ):
				if ( nDeltaX > 2038 / self.step_scaling_factor ):
					while ( nDeltaX > 2038 / self.step_scaling_factor ):
						nDeltaX -= 4076 / self.step_scaling_factor
				elif ( nDeltaX < -2038 / self.step_scaling_factor ):
					while ( nDeltaX < -2038 / self.step_scaling_factor ):
						nDeltaX += 4076 / self.step_scaling_factor
